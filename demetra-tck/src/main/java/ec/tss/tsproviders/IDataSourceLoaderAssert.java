...
import static ec.tss.tsproviders.Utils.BAD_BEAN;
import static ec.tss.tsproviders.Utils.BAD_DATA_SOURCE;
import static ec.tss.tsproviders.Utils.NULL_BEAN;
...
/**
 * {@link IDataSourceLoader} specific assertions.
 *
...
public class IDataSourceLoaderAssert extends AbstractAssert<IDataSourceLoaderAssert, IDataSourceLoader> {
    ...
    /**
     * Creates a new <code>{@link IDataSourceLoaderAssert}</code> to make
    ...
    /**
     * Verifies that the actual IDataSourceLoader's asyncMode is equal to the
     * given one.
     *
     * @param asyncMode the given asyncMode to compare the actual
     * IDataSourceLoader's asyncMode to.
    ...
    private static <T extends IDataSourceLoader> void checkOpen(SoftAssertions s, Supplier<T> factory, Sampler<T> sampler) {
        try (T p = factory.get()) {
            s.assertThatThrownBy(() -> p.open(NULL_DATA_SOURCE))
                    .as(throwDescription(p, "open(nullDataSource)", NullPointerException.class))
                    .isInstanceOf(NullPointerException.class);
            s.assertThatThrownBy(() -> p.open(BAD_DATA_SOURCE))
                    .as(throwDescription(p, "open(badDataSource)", IllegalArgumentException.class))
    ...
    private static <T extends IDataSourceLoader> void checkClose(SoftAssertions s, Supplier<T> factory, Sampler<T> sampler) {
        try (T p = factory.get()) {
            s.assertThatThrownBy(() -> p.close(NULL_DATA_SOURCE))
                    .as(throwDescription(p, "close(nullDataSource)", NullPointerException.class))
                    .isInstanceOf(NullPointerException.class);
            s.assertThatThrownBy(() -> p.close(BAD_DATA_SOURCE))
                    .as(throwDescription(p, "close(badDataSource)", IllegalArgumentException.class))
                    .isInstanceOf(IllegalArgumentException.class);
            sampler.dataSource(p).ifPresent(o -> {
                p.open(o);
                p.close(o);
    ...
    private static <T extends IDataSourceLoader> void checkCloseAll(SoftAssertions s, Supplier<T> factory, Sampler<T> sampler) {
    }

    private static <T extends IDataSourceLoader> void checkNewBean(SoftAssertions s, Supplier<T> factory, Sampler<T> sampler) {
        try (T p = factory.get()) {
    ...
    private static <T extends IDataSourceLoader> void checkDecodeBean(SoftAssertions s, Supplier<T> factory, Sampler<T> sampler) {
        try (T p = factory.get()) {
            s.assertThatThrownBy(() -> p.decodeBean(NULL_DATA_SOURCE))
                    .as(throwDescription(p, "decodeBean(nullDataSource)", NullPointerException.class))
                    .isInstanceOf(NullPointerException.class);
            s.assertThatThrownBy(() -> p.decodeBean(BAD_DATA_SOURCE))
                    .as(throwDescription(p, "decodeBean(badDataSource)", IllegalArgumentException.class))
                    .isInstanceOf(IllegalArgumentException.class);
            sampler.dataSource(p).ifPresent(o -> {
    ...
    private static <T extends IDataSourceLoader> void checkEncodeBean(SoftAssertions s, Supplier<T> factory, Sampler<T> sampler) {
        try (T p = factory.get()) {
            s.assertThatThrownBy(() -> p.encodeBean(NULL_BEAN))
                    .as(throwDescription(p, "encodeBean(nullBean)", NullPointerException.class))
                    .isInstanceOf(NullPointerException.class);
            s.assertThatThrownBy(() -> p.encodeBean(BAD_BEAN))
                    .as(throwDescription(p, "encodeBean(badBean)", IllegalArgumentException.class))
                    .isInstanceOf(IllegalArgumentException.class);
    ...
    private static DataSet firstSeriesOrNull(IDataSourceProvider p, DataSource target) {
        AtomicReference<DataSet> result = new AtomicReference<>();
        try {
            TsProviders.walkProviderTree(p, new IProviderVisitor<IDataSourceProvider>() {
                @Override
                public boolean preVisitSource(IDataSourceProvider provider, DataSource dataSource) throws IOException {
                    return dataSource.equals(target);
                }

                @Override
                public boolean visitSeries(IDataSourceProvider provider, DataSet dataSet, int level) throws IOException {
                    result.set(dataSet);
                    return false;
                }
            });
    ...
    private static DataSet firstCollectionOrNull(IDataSourceProvider p, DataSource target) {
        AtomicReference<DataSet> result = new AtomicReference<>();
        try {
            TsProviders.walkProviderTree(p, new IProviderVisitor<IDataSourceProvider>() {
                @Override
                ...
                @Override
                public boolean preVisitCollection(IDataSourceProvider provider, DataSet dataSet, int level) throws IOException {
                    result.set(dataSet);
                    return false;
                }
            });
