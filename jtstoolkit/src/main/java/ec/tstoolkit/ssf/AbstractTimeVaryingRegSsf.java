...
public abstract class AbstractTimeVaryingRegSsf implements ISsf {
    ...
    protected abstract void fullRegNoise(int pos, SubMatrix vregs);

    protected abstract int getRegNoiseDim();

    protected abstract boolean isRegNoiseTimeInvariant();

    protected abstract void regNoise(int pos, SubMatrix vregs);
     ...
     * @param vregs
     */
    protected abstract void wRegNoise(int pos, SubMatrix vregs);
     ...
     * @param X
     */
    public AbstractTimeVaryingRegSsf(final ISsf ssf, final SubMatrix X) {
        m_ssf = ssf;
        m_X = X;
    ...
    public void fullQ(final int pos, final SubMatrix qm) {
        ...
        fullRegNoise(pos, qm.extract(m_mr, m_r, m_mr, m_r));
    }
    ...
    public int getTransitionResCount() {
        return m_ssf.getTransitionResCount() + m_X.getColumnsCount();
    }
    ...
    public int getTransitionResDim() {
        return m_ssf.getTransitionResDim() + getRegNoiseDim();
    }
    ...
    @Override
    public boolean hasW() {
        return m_ssf.hasW() || m_X.getColumnsCount() > getRegNoiseDim();
    }
    ...
    public boolean isTransitionEquationTimeInvariant() {
        return m_ssf.isTransitionEquationTimeInvariant() && isRegNoiseTimeInvariant();
    }
    ...
    public boolean isTransitionResidualTimeInvariant() {
        return m_ssf.isTransitionResidualTimeInvariant() && isRegNoiseTimeInvariant();
    }
    ...
    public void Q(final int pos, final SubMatrix qm) {
        int r = m_ssf.getTransitionResDim(), rd = getRegNoiseDim();
        m_ssf.Q(pos, qm.extract(0, r, 0, r));
        regNoise(pos, qm.extract(r, r+rd, r, r+rd));
    }
    ...
    public void R(final int pos, final SubArrayOfInt rv) {
        if (m_ssf.hasR()) {
            int r = m_ssf.getTransitionResCount();
            m_ssf.R(pos, rv.range(0, r));
            for (int i = m_mr; i < m_r; ++i) {
                rv.set(r++, i);
            }
        }
    }
    ...
    public void W(final int pos, final SubMatrix wv) {
        int mr = m_ssf.getTransitionResCount(), mc = m_ssf.getTransitionResDim();
        int xr = m_X.getColumnsCount(), xc = getRegNoiseDim();
        if (mc < mr) {
            m_ssf.W(pos, wv.extract(0, mr, 0, mc));
        }
        if (xr < xc) {
            wRegNoise(pos, wv.extract(mr, mr + xr, mc, mc + xc));
        }else{
            wv.subDiagonal(mc-mr).drop(mc, 0).set(1);
        }
    }
