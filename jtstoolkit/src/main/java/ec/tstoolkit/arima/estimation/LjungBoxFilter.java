...
public class LjungBoxFilter implements IArmaFilter {
    ...
    public int initialize(IArimaModel arima, int n) {
        ...
        if (m > 0) {
            ...
            m_L = new Matrix(m_p + m_q, m_p + m_q);
            m_L.diagonal().set(1);
            m_V1 = new Matrix(m, m_p + m_q);
            if (m_p > 0) {
                double[] cov = arima.getAutoCovarianceFunction().values(m_p);
                SubMatrix W = m_L.subMatrix(0, m_p, 0, m_p);
                W.diagonal().set(cov[0]);

                for (int i = 1; i < m_p; ++i) {
                    W.subDiagonal(i).set(cov[i]);
                }
                SubMatrix P = m_V1.subMatrix(0, m_p, 0, m_p);
            ...
            if (m_q > 0) {
                SubMatrix Q = m_V1.subMatrix(0, m_q, m_p, m_p + m_q);
            ...
            if (m_q > 0 && m_p > 0) {
                double[] psi = new RationalFunction(m_ma, m_ar).coefficients(m_q);
                SubMatrix W = m_L.subMatrix(0, m_p, m_p, m_p + m_q);
                int imin = m_q - m_p;
                for (int i = 0; i < m_q; ++i) {
            ...
            Matrix I = new Matrix(m_p + m_q, m_p + m_q);
            I.diagonal().set(1);
