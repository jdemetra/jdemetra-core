...
public class AutoCovarianceFunction {
    ...
    public void setMethod(Method method) {
        if (ma_ == null && (method == Method.Default || method == Method.Default2)) {
    ...
    private void computeDefault(int rank) {
        int p = ar_.getDegree() + 1;
        int q = ma_.getDegree() + 1;
        int r0 = Math.max(p, q);
        if (rank < r0) {
            rank = r0;
        }
        ...
        if (c_ == null) {
            ...
            RationalFunction rfe = new RationalFunction(ma_, ar_);
    ...
    private void computeSymmetric(int rank, boolean dsym) {
        int p = ar_.getDegree() + 1;
        int q = sma_ != null ? sma_.getDegree() + 1 : ma_.getDegree() + 1;
        int r0 = Math.max(p, q);
        if (rank < r0) {
            rank = r0;
        }
        if (p == 1) {
            // pure moving average...
            if (sma_ == null) {
                sma_ = SymmetricFilter.createFromFilter(new BackFilter(ma_));
            }
            ...
            if (c_ == null) {
                ...
                BackFilter g = dsym ? sma_.decompose(new BackFilter(ar_)) : sma_.decompose2(new BackFilter(ar_));
                double[] tmp = new RationalFunction(g.getPolynomial(), ar_).coefficients(rank + 1);
