...
/**
 * The singularValueDecomposition decomposes a matrix M in
 * M = U * S * V'
 * with:
 * U'U = I
 * S diagonal with the singular values (ordered by size)
 * V'V = I
 * 
 * M ~ m x n (n <= m)
 * U ~ m x n
 ...
 * V ~ n x n
 * 
 * The columns of V are the principal components of M
 * The columns of U * D are the principal factors of M
 * We also have:
 * M' = V * S * U'
 * M' * M = V * S * U' * U * S * V' = V * S^2 * V'
 * ( the columns of V are the eigen vectors of M'M)
 * 
 * If we set F = U * S
 * we have:
 * M = F * V' (the rows of V are the weights of the factors F for decomposing M)
 * M V = F (the columns of V are the weights applied on the columns of M to get the factors F)
...
public class SingularValueDecomposition extends AbstractLinearSystemSolver {
    ...
    @Override
    public void decompose(Matrix arg) throws MatrixException {
        Matrix A = arg.clone();
        init(A);
    }

    @Override
    public void decompose(SubMatrix arg) {
        Matrix A = new Matrix(arg);
        init(A);
    }

    private void init(Matrix matrix) {
        double[] A = matrix.internalStorage();
        ...
        for (int k = 0, mk = 0; k < Math.max(nct, nrt); k++, mk += m_m) {
            if (k < nct) {
                ...
                for (int i = mk + k; i < mk + m_m; i++) {
                    m_s[k] = DataBlock.hypot(m_s[k], A[i]);
                }
                if (m_s[k] != 0.0) {
                    if (A[k + mk] < 0.0) {
                        m_s[k] = -m_s[k];
        ...
        while (p > 0) {
            ...
            switch (kase) {
                ...
                case 1: {
                    ...
                    e[p - 2] = 0.0;
                    for (int j = p - 2; j >= k; j--) {
                        double t = DataBlock.hypot(m_s[j], f);
                        double cs = m_s[j] / t;
                        double sn = f / t;
                        m_s[j] = t;
                        if (j != k) {
    ...
    @Override
    public double[] solve(double[] b) throws MatrixException {
        if (b.length != m_m) {
    ...
    public void solve(DataBlock xin, DataBlock xout) throws MatrixException {
        double[] data = new double[xin.getLength()];
        xin.copyTo(data, 0);
    ...
    public void solve(Matrix B, Matrix X) {
        DataBlockIterator bcols = B.columns(), xcols = X.columns();
        DataBlock b = bcols.getData(), x = xcols.getData();
        do {
            solve(b, x);
        } while (bcols.next() && xcols.next());
    }
}
