            ...
            if (l > 0) {
                ...
                if (scale == 0.0) {
                    ...
                    for (int j = 0, idx3 = i * n, idx5 = i, idx4 = j * n + i; j < l + 1; j++, idx3++, idx4 += n, idx5 += n) {
                        ...
                        for (int k = j + 1, idxj = j * n + k, idxi = k * n + i; k < l + 1; k++, idxj++, idxi += n) // check idxi should start at i,k
                        {
                            g += sm[idxj] * sm[idxi];
                        }
                        ev[j] = g / h;
    ...
    static void triQL(double[] ev, int n, double[] zz, boolean bVec) {
        ...
        for (int l = 0, idxld = n, idxle = 0; l < n; l++, idxld++, idxle++) {
            ...
            do {
                ...
                if (m != l) {
                    if (iter++ == m_maxiter) {
                        throw new MatrixException(MatrixException.EigenFailed);
                    }
