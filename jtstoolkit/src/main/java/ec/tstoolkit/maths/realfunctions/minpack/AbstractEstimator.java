...
public abstract class AbstractEstimator implements IEstimator {
     ...
     * parameters can be retrieved from the problem itself, through the
     * {@link EstimationProblem#getAllParameters
    ...
    protected void updateResidualsAndCost(IEstimationProblem problem) {
        if (++m_costEvaluations > m_maxCostEval) {
            throw new FunctionException(FunctionException.MAXITER_ERR,
                    "MinPack");
        }

        m_cost = 0;
        for (int i = 0, index = 0; i < m_rows; i++, index += m_cols) {
            double residual = problem.getResidual(i);
