...
public final class Complex implements Serializable {
    ...
    public final static class SimplifyingTool extends Simplifying<Complex[]> {
	 ...
	 */
	@Override
	public boolean simplify(final Complex[] left, final Complex[] right) {
	    clear();
	    if (Arrays2.isNullOrEmpty(left) || Arrays2.isNullOrEmpty(right))
		return false;

	    boolean[] lc = new boolean[left.length], rc = new boolean[right.length];
	    int nc = 0;
	    for (int i = 0; i < left.length; ++i)
		for (int j = 0; j < right.length; ++j)
		    if (!rc[j]
			    && ((left[i].minus(right[j])).abs() <= eps_)) {
			lc[i] = true;
			rc[j] = true;
			++nc;
			break;
	    ...
	    Complex[] nlr = null, nrr = null, ncr = null;
	    if (nc != left.length) {
		ncr = new Complex[nc];
		nlr = new Complex[left.length - nc];
		for (int i = 0, j = 0, k = 0; i < lc.length; ++i)
		    if (lc[i])
			ncr[k++] = left[i];
		    else
			nlr[j++] = left[i];
 ...
 */
     ...
     * @param lrlength
     *            The number of roots in lroots to take into account
     * @param rr
     *            An array of complex roots
     * @param rrlength
     *            The number of roots in lroots to take into account
     ...
     *            An array of complex roots
     * @param epsilon
     *            Contains the number of items in the intersection on return
...
//		y1 = ytmp;
    ...
    @Override
    public int hashCode() {
        return 31 * (31 * 1 + Jdk6.Double.hashCode(re)) + Jdk6.Double.hashCode(im);
    }
