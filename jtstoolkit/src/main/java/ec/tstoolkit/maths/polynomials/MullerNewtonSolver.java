...
public class MullerNewtonSolver implements IRootsSolver {
     ...
     */
    Complex q2;
    final Ref<Complex> f0, f1, f2; /* of parabola and polynomial */
    ...
    private void check_x_value(final Ref<Complex> xb,
        ...
        if ((f2absq <= (MBOUND1 * f1absq)) && (f2absq >= (MBOUND2 * f1absq))) /* function-value changes slowly */ {
            ...
            f2absqb.val = f2absq; /* the new function value is the */
    ...
    private void initialize(final Ref<Complex> xb, final DoubleRef epsilon) /* Complex *pred, coefficient vector of the deflated polynomial */ /* *xb; best x-value */ /* double *epsilon; bound for |q2| */ {
        ...
        x0 = Complex.ZERO; /* x0 = 0 + j*1 */
        x1 = Complex.cart(-ISQRT2, -ISQRT2); /* x1 = 0 - j*1 */
        x2 = Complex.cart(ISQRT2, ISQRT2); /* x2 = (1 + j*1)/Sqrt(2) */

        h1 = x1.minus(x0); /* h1 = x1 - x0 */
        h2 = x2.minus(x1); /* h2 = x2 - x1 */
        q2 = h2.div(h1); /* q2 = h2/h1 */

        xb.val = x2; /* best initial x-value = zero */
    ...
    private void iteration_equation(final DoubleRef h2abs) /* double *h2abs; Absolute value of the old distance */ {
        ...
        h2abs.val = h2absnew; /* actualize old distance for next iteration */
     ...
     */
    private boolean lin_or_quad() /* Complex *pred, coefficient vector of the deflated polynomial */ /* *root; determined roots */ /* int nred; highest exponent of the deflated polynomial */ {
        int nred = m_pred.length - m_idx - 1;
        if (nred == 1) { /* root = -p0/p1 */
            m_roots[m_idx] = Complex.cart(-m_pred[m_idx] / m_pred[m_idx + 1]);
            return true; /* and return no error */
        } else if (nred == 2) { /* quadratic polynomial */
            quadratic();
            return true; /* return no error */
        }
        return false; /* nred>2 => no roots were calculated */
    }
	...
	/* with highest exponent */
        int n = m_p.length - 1;
        double factor = Math.abs(1 / m_p[n]); /* factor = |1/pn| */
    ...
    private Complex muller() {
        double f1absq = MFVALUE; /* f1absq=|f1|^2 */
        final DoubleRef f2absq = new DoubleRef(MFVALUE); /* f2absq=|f2|^2 */
        final DoubleRef f2absqb = new DoubleRef(MFVALUE); /*
         * f2absqb=|P(xb)|^
         * 2
         */
        final DoubleRef h2abs = new DoubleRef(0d); /* h2abs=|h2| */
        final DoubleRef epsilon = new DoubleRef(0d); /* bound for |q2| */
        final IntRef seconditer = new IntRef(0); /*
         * second
         * iteration, when
         * root is too bad
         */
        final IntRef noise = new IntRef(0); /* noise counter */
        ...
        fdvalue(m_pred, m_idx, f0, x0); /* compute exact function value */
        fdvalue(m_pred, m_idx, f1, x1); /* oct-29-1993 ml */
        fdvalue(m_pred, m_idx, f2, x2);
    ...
    private Complex newton(final Complex ns, final DoubleRef dxabs) /* ns; determined root with Muller method */ /* double *dxabs; dxabs = |P(x0)/P'(x0)| */ {
        ...
        for (int i = 0; i < NITERMAX; i++) { /* main loop */
            ...
            if (axmin != 0) {
                if ((dxabs.val / axmin < eps) || (noise == NNOISEMAX)) {
                    ...
                    dxabs.val /= axmin; /* return relative error */
    ...
    private boolean newtonnull() /* Complex *p, coefficient vector of the original polynomial */ /* *pred, coefficient vector of the deflated polynomial */ /* *root; determined roots */ /* int *n; the highest exponent of the original polynomial */ /* double *maxerr; max. error of all determined roots */ /* unsigned char flag; flag = 1 => Complex coefficients */ /* flag = 0 => real coefficients */ {
        // newerr; error of actual root
        // ns; root determined by Muller's method
        // nred, highest exponent of deflated polynomial
        // error; indicates an error in poly_check
        // diff; number of roots at 0

        final DoubleRef newerr = new DoubleRef(0d);
        m_maxerr = 0; /* initialize max. error of determined roots */
...
//	    m_maxerr = newerr.val;
    ...
    private void root_check(final double f2absqb,
        ...
        if ((seconditer.val == 1) && (f2absqb > 0)) {
            ...
            if (f2.val.abs() / (df.val.abs() * xb.abs()) > MBOUND7) {
                ...
                x1 = Complex.NEG_ONE;
                x2 = Complex.ZERO; /*   */
                fdvalue(m_pred, m_idx, f0, x0); /* f0 = P(x0) */
                fdvalue(m_pred, m_idx, f1, x1); /* f1 = P(x1) */
                fdvalue(m_pred, m_idx, f2, x2); /* f2 = P(x2) */
                iter = 0; /* reset iteration counter */
                ++seconditer.val; /* increase seconditer */
                rootd.val = false; /* no root determined */
                noise.val = 0; /* reset noise counter */
            }
        }
    }
    ...
    private void too_big_functionvalues(final DoubleRef f2absq) /* double *f2absq; f2absq=|f2|^2 */ {
        if ((Math.abs(f2.val.getRe()) + Math.abs(f2.val.getIm())) > MBOUND4)  {
            ...
            f2absq.val = f2.val.absSquare(); /* |f2|^2 = f2.r^2+f2.i^2 */
        }
    }
    ...
    private void update(final Complex r0) {
        ...
        if (!lqdiv) {
            ...
            Polynomial num = Polynomial.copyOf(m_pred, m_idx-2, m_degree+ 1);
            Polynomial div = Polynomial.of(new double[]{-b, -a, 1});
    ...
    private void update(final double r0) {
        ...
        if (!lqdiv) {
            ...
            Polynomial num = Polynomial.copyOf(m_pred, m_idx-1, m_degree + 1);
            Polynomial div = Polynomial.of(new double[]{-r0, 1});
            LeastSquaresDivision lq = new LeastSquaresDivision();
            lq.divide(num, div);
            lq.getQuotient().copyTo(m_pred, m_idx);
        }
    }
    private boolean lqdiv;
