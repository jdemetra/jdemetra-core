...
public class GrantHitchinsSolver implements IRootsSolver {
    ...
    private ec.tstoolkit.maths.Complex[] m_roots;
	...
	while (++j < g_max) {
		...
		s1 = P5 * s1;
		s2 = P5 * s2;
		if (Math.abs(s1) <= g_eps * Math.abs(m_x)
			&& Math.abs(s2) <= g_eps * Math.abs(m_y))
		    return false;
		s *= P5;
		sig *= P5;
		m_x -= s1;
	    ...
	    if (k == g_max)
		throw new ec.tstoolkit.maths.polynomials.PolynomialException(
	...
	if (n != 1)
	    n = solve(a, n, rez, imz, true, g_eps2);
	if (n <= 1)
	    return true;
	else
	    return false;
