...
public class HannanRissanenBis {
     ...
     *
     */
    public HannanRissanenBis() {
    }

    /**
     * See Chen Zhao-Guo Journal of time series analysis [1985]
     */
    private void biascorrection() {
        ...
        for (int i = 1; i <= m_spec.getP(); ++i) {
            mat.column(cur++).drop(i, 0).copyFrom(a1, 0);
        }
        for (int i = 1; i <= m_spec.getBP(); ++i) {
            mat.column(cur++).drop(i * m_spec.getFrequency(), 0).copyFrom(a1, 0);
            for (int j = 1; j <= m_spec.getP(); ++j) {
                mat.column(cur++).drop(j + i * m_spec.getFrequency(), 0).copyFrom(a1, 0);
            }
        }
        for (int i = 1; i <= m_spec.getQ(); ++i) {
            mat.column(cur++).drop(i, 0).copyFrom(a2, 0);
        }
        for (int i = 1; i <= m_spec.getBQ(); ++i) {
            mat.column(cur++).drop(i * m_spec.getFrequency(), 0).copyFrom(a2, 0);
            for (int j = 1; j <= m_spec.getQ(); ++j) {
                mat.column(cur++).drop(j + i * m_spec.getFrequency(), 0).copyFrom(a2, 0);
            }
        }
        ...
        try {
            ...
            qr.decompose(mat);
            qr.setEpsilon(EPS);
            double[] dpi = qr.solve(res); // pi contains -phi, theta
...
//        }
    ...
    private void finalcorrection() {
        ...
        HannanRissanenBis hr = new HannanRissanenBis();
...
//        }
        ...
        DataBlock rc = new DataBlock(m_data, nar, n, 1);
        ...
        Householder qr = new Householder(false);
        qr.decompose(M);
        double[] ar = new double[nar];
        DataBlock c = new DataBlock(ar);
        DataBlock e = new DataBlock(nc - nar);
        qr.leastSquares(new DataBlock(m_data, nar, n, 1), c, e);
        for (int i = 0; i < n; ++i) {
            double z = m_data[i];
            int jmax = nar > i ? i : nar;
            for (int j = 1; j <= jmax; ++j) {
                z -= ar[j - 1] * m_data[i - j];
            }
            m_a[i] = z;
    ...
    // step 1 of the process ... regression
    private void minspq() {
        int p = m_spec.getP() + m_spec.getFrequency() * m_spec.getBP();
        int q = m_spec.getQ() + m_spec.getFrequency() * m_spec.getBQ();
        int n = m_data.length;
