...
//        addOutliersHistory(context);
    ...
    public PreprocessingModel process(TsData originalTs, ModellingContext context) {
        ...
        context.description = new ModelDescription(originalTs, estimationSpan == null ? null : originalTs.getDomain().select(estimationSpan));
    ...
    protected boolean pass2(final boolean same, ModellingContext context) {
        double fct = 1, fct2 = 1;
...
//    }
    ...
    private void checkSeasonality(ModellingContext context) {
        if (seas != null) {
            seas.process(context);
        } else {
            context.hasseas = checkSeasonality(context.description.transformedOriginal());
        }
