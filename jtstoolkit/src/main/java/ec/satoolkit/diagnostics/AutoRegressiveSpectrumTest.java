...
public class AutoRegressiveSpectrumTest {
    ...
    private double[] f = new double[NFREQ + 1], spectrum = new double[NFREQ + 1];
    private int arcount;
    ...
    // settings
    public boolean test(TsData data) {
        if (!computeARSpectrum(data)) {
            return false;
        }
        computeFrequencies(data.getFrequency().intValue());
        computeRange();
        return true;
    }
    ...
    private boolean computeARSpectrum(TsData data) {
        int freq = data.getFrequency().intValue();
        int n = data.getLength();
        int nar = arcount(n, freq);
        if (nar >= n) {
            return false;
        }
    ...
    private void computeRange() {
        DescriptiveStatistics stats = new DescriptiveStatistics(spectrum);
