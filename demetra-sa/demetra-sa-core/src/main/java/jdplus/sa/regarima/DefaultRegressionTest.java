...
//    private IRegressionTest tdTest_, wdTest_, lpTest_, mhTest_, meanTest_;
    ...
    private ModelDescription createTestModel(RegSarimaModelling current) {
        ...
        if (td != null) {
            model.addVariable(Variable.variable("td", td, ModelBuilder.calendarAMI));
        }
        if (lp != null) {
            model.addVariable(Variable.variable("lp", lp, ModelBuilder.calendarAMI));
        }
        if (easter != null) {
            model.addVariable(Variable.variable("easter", easter, ModelBuilder.calendarAMI));
        }
        return model;
    }
