...
public class CsvSummary {
    ...
    public CsvSummary(String[] items, String name, SaDocument<?> document) {
        ...
        series_.put("y", s);
        domain_ = s.getDomain();
        fdomain_ = new TsDomain(domain_.getEnd(), domain_.getFrequency().intValue());
    ...
    private void fillDictionary(String[] items, IProcResults results) {
        for (String item : items) {
            if (item.equalsIgnoreCase("y"))
                ...
                series_.put("yl", results.getData(ModellingDictionary.Y_CMP, TsData.class));
            else if (item.equalsIgnoreCase("t"))
                series_.put("t", results.getData(ModellingDictionary.T, TsData.class));
            else if (item.equalsIgnoreCase("sa"))
                series_.put("sa", results.getData(ModellingDictionary.SA, TsData.class));
            else if (item.equalsIgnoreCase("s"))
                series_.put("s", results.getData(ModellingDictionary.S, TsData.class));
            else if (item.equalsIgnoreCase("i"))
                series_.put("i", results.getData(ModellingDictionary.I, TsData.class));
            else if (item.equalsIgnoreCase("tc"))
                series_.put("tc", results.getData(ModellingDictionary.T_CMP, TsData.class));
            else if (item.equalsIgnoreCase("sac"))
                series_.put("sac", results.getData(ModellingDictionary.SA_CMP, TsData.class));
            else if (item.equalsIgnoreCase("sc"))
                series_.put("sc", results.getData(ModellingDictionary.S_CMP, TsData.class));
            else if (item.equalsIgnoreCase("ic"))
                series_.put("ic", results.getData(ModellingDictionary.I_CMP, TsData.class));
            else if (item.equalsIgnoreCase("tl"))
                series_.put("tc", results.getData(ModellingDictionary.T_LIN, TsData.class));
            else if (item.equalsIgnoreCase("sal"))
                series_.put("sac", results.getData(ModellingDictionary.SA_LIN, TsData.class));
            else if (item.equalsIgnoreCase("sl"))
                series_.put("sc", results.getData(ModellingDictionary.S_LIN, TsData.class));
            else if (item.equalsIgnoreCase("il"))
                series_.put("ic", results.getData(ModellingDictionary.I_LIN, TsData.class));
