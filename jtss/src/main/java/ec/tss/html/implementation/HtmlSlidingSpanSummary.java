...
public class HtmlSlidingSpanSummary extends AbstractHtmlElement implements IHtmlElement {
    ...
    public void write(HtmlStream stream) throws IOException {
        ...
        stream.write("Sliding spans summary", HtmlStyle.Bold, HtmlStyle.Underline).newLines(2);
        stream.write("Time spans", HtmlStyle.Bold).newLines(2);
        ...
        stream.newLine().write("Tests for seasonality", HtmlStyle.Bold).newLines(2);
        stream.open(new HtmlTable(0, 400));
        stream.open(HtmlTag.TABLEROW);
        stream.write(new HtmlTableCell(""));
        for (int i = 1; i <= ncols; ++i)
            stream.write(new HtmlTableCell("Span " + i, HtmlStyle.Italic));
        stream.close(HtmlTag.TABLEROW);
        stream.open(HtmlTag.TABLEROW);
        stream.write(new HtmlTableCell("Stable seas.", HtmlStyle.Italic));
        for (int i = 0; i < ncols; ++i)
            stream.write(SeasonalityCell(tests[i].getStableSeasonality()));
        stream.close(HtmlTag.TABLEROW);
        stream.open(HtmlTag.TABLEROW);
        stream.write(new HtmlTableCell("Kruskal-Wallis", HtmlStyle.Italic));
        for (int i = 0; i < ncols; ++i)
            stream.write(SeasonalityCell(tests[i].getNonParametricTestForStableSeasonality()));
        stream.close(HtmlTag.TABLEROW);
        stream.open(HtmlTag.TABLEROW);
        stream.write(new HtmlTableCell("Moving seas.", HtmlStyle.Italic));
        for (int i = 0; i < ncols; ++i)
            stream.write(MovingSeasonalityCell(tests[i].getEvolutiveSeasonality()));
        stream.close(HtmlTag.TABLEROW);
        stream.open(HtmlTag.TABLEROW);
        stream.write(new HtmlTableCell("Identifiable seas.", HtmlStyle.Italic));
        for (int i = 0; i < ncols; ++i)
        ...
        stream.newLines(2).write("Means of seasonal factors", HtmlStyle.Bold).newLines(2);
        stream.open(new HtmlTable(0, 400));
        stream.open(HtmlTag.TABLEROW);
        stream.write(new HtmlTableCell(""));
        for (int i = 1; i <= ncols; ++i)
    ...
    private HtmlTableCell SeasonalityCell(StatisticalTest test) {
        ...
        if (test.getPValue() > .05)
            return new HtmlTableCell(val, HtmlStyle.Bold, HtmlStyle.Danger);
        else if (test.getPValue() > .01)
            return new HtmlTableCell(val, HtmlStyle.Warning);
        else
            return new HtmlTableCell(val, HtmlStyle.Success);
    }
    ...
    private HtmlTableCell IdentifiableCell(CombinedSeasonalityTest test) {
        switch(test.getSummary()) {
            case None:
                return new HtmlTableCell("NO", HtmlStyle.Bold, HtmlStyle.Danger);
            case ProbablyNone:
                return new HtmlTableCell("???", HtmlStyle.Warning);
            case Present:
                return new HtmlTableCell("YES", HtmlStyle.Success);
        }
        return null;
    }
}
