...
public class SubTableOfInt {
     ...
     * @param ncols
     */
    public SubTableOfInt(final int[] data, final int nrows, final int ncols) {
	m_data = data;
	m_nrows = nrows;
	m_ncols = ncols;
	m_row_inc = 1;
     ...
     * @param colinc
     */
    public SubTableOfInt(final int[] data, final int start, final int nrows,
     ...
     * @param m
     */
    public SubTableOfInt(final SubTableOfInt m) {
	m_data = m.m_data;
	m_start = m.m_start;
	m_nrows = m.m_nrows;
	m_ncols = m.m_ncols;
     ...
     * @return
     */
    public SubArrayOfInt column(final int c) {
	int beg = m_start + c * m_col_inc, end = beg + m_row_inc * m_nrows;
	return SubArrayOfInt.create(m_data, beg, end, m_row_inc);
    }
     ...
     * @return
     */
    public SubArrayOfInt diagonal() {
	int n = Math.min(m_nrows, m_ncols), inc = m_row_inc + m_col_inc;
	return SubArrayOfInt.create(m_data, m_start, m_start + inc * n, inc);
    }
    ...
    public SubTableOfInt extract(final int r0, final int r1, final int c0,
	    final int c1) {
	return new SubTableOfInt(m_data, m_start + r0 * m_row_inc + c0
     ...
     * @return
     */
    public SubTableOfInt extract(final int r0, final int c0, final int nrows,
	    final int ncols, final int rowinc, final int colinc) {
	return new SubTableOfInt(m_data, m_start + r0 * m_row_inc + c0
     ...
     * @return
     */
    public SubArrayOfInt row(final int r) {
	int beg = m_start + r * m_row_inc, end = beg + m_col_inc * m_ncols;
	return SubArrayOfInt.create(m_data, beg, end, m_col_inc);
    }
     ...
     * @return
     */
    public SubArrayOfInt subDiagonal(int pos) {
	if (pos >= m_ncols)
	    return SubArrayOfInt.EMPTY;
	if (-pos >= m_nrows)
	    return SubArrayOfInt.EMPTY;
	...
	return SubArrayOfInt.create(m_data, beg, beg + inc * n, inc);
    }
     ...
     * @return
     */
    public SubTableOfInt transpose() {
	return new SubTableOfInt(m_data, m_start, m_ncols, m_nrows, m_col_inc,
		m_row_inc);
    }
