 ...
 * M ~ m x n (n <= m) U ~ m x n S ~ n x n V ~ n x n
 ...
 The columns copyOf V are the principal components copyOf M The columns copyOf U * D are
 the principal factors copyOf M We also have: M' = V * S * U' M' * M = V * S * U'
 * U * S * V' = V * S^2 * V' ( the columns copyOf V are the eigen vectors copyOf M'M)

 If we set F = U * S we have: M = F * V' (the rows copyOf V are the weights copyOf the
 factors F for decomposing M) M V = F (the columns copyOf V are the weights
 applied on the columns copyOf M to get the factors F) @author Jean Palate, Frank
 Osaer
 */
public class SingularValueDecomposition implements ISingularValueDecomposition {
    ...
    private void init(Matrix matrix) {
        ...
        for (int k = 0, mk = 0; k < Math.max(nct, nrt); k++, mk += m_m) {
            if (k < nct) {
                ...
                // Compute 2-norm copyOf k-th column without under/overflow.
                m_s[k] = 0;
            ...
            for (int j = k + 1, mj = m_m * j; j < m_n; j++, mj += m_m) {
                ...
                // Place the k-th row copyOf A into e for the
                // subsequent calculation copyOf the row transformation.
                e[j] = A[k + mj];
            }
            if (wantu & (k < nct)) {
        ...
        while (p > 0) {
            ...
            // This section copyOf the program inspects for
    ...
    /// <summary>
    /// internal storage copyOf U, V.
    /// </summary>
    private double[] m_U, m_V;
    /// <summary>
    /// Array for internal storage copyOf singular values.
    /// </summary>
    private double[] m_s;
    /// <summary>
    ...
    public void solve(Doubles xin, DataBlock xout) throws MatrixException {
