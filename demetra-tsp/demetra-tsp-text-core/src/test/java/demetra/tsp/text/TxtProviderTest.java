...
public class TxtProviderTest {
    ...
    public void testMonikerLegacy() {
        ...
        try (TxtProvider p = new TxtProvider()) {
            assertThat(p.toDataSet(TsMoniker.of("Txt", legacy))).hasValue(expected);
        }
    }
    ...
    public void testMonikerUri() {
        ...
        try (TxtProvider p = new TxtProvider()) {
            assertThat(p.toDataSet(TsMoniker.of("Txt", uri))).hasValue(expected);
        }
    }
    ...
    public void testSample() throws IOException {
        try (TxtProvider p = TxtSamples.INSEE1.getProvider3()) {
            TxtBean bean = TxtSamples.INSEE1.getBean3(p);
            ...
            assertThat(p.getTs(p.toMoniker(leaf), TsInformationType.All))
                    .satisfies(o -> {
                        ...
                        assertThat(o.getName()).isEqualTo("S854655");
                        assertThat(o.getMeta()).isEmpty();
                        assertThat(o.getData()).isEqualTo(TsData.ofInternal(TsPeriod.monthly(1990, 1), VALUES));
