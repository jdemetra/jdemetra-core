...
public class SarimaModel extends AbstractArimaModel {
    ...
    public boolean isInvertible() {
        return Utilities.checkStability(getRegularMA())
                && Utilities.checkStability(getSeasonalMA());
    }
    ...
    public boolean isStable(boolean checkMA) {
        int pos = 0;
        if (phi.length > 0 && !Utilities.checkStability(DataBlock.ofInternal(phi))) {
            return false;
        }
        if (bphi.length > 0 && !Utilities.checkStability(DataBlock.ofInternal(bphi))) {
            return false;
        }
        if (checkMA) {
            if (th.length > 0 && !Utilities.checkStability(DataBlock.ofInternal(th))) {
                return false;
            }
            if (bth.length > 0 && !Utilities.checkStability(DataBlock.ofInternal(bth))) {
                return false;
            }
        }
        return true;
