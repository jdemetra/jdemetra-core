...
//        }
        ...
        System.out.println(monitor.fullParameters());
        StateStorage ss = DkToolkit.smooth(monitor.getSsf(), new SsfMatrix(monitor.getData()), true);
        System.out.println(ss.getComponent(monitor.getSsf().componentsPosition()[4]+4));
    }
    ...
    private void generateCycle(MstsMapping mapping) {
        mapping.add(new ArParameters("ar", 2, null, new double[]{1, -.5}));
    ...
    private void generateXCycle(MstsMapping mapping) {
        mapping.add(new ArParameters("ar", 2, null, new double[]{.1, .1}));
        mapping.add(new VarianceParameter("ar_var", 1));
