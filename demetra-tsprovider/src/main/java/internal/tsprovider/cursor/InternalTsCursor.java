...
import demetra.io.PredicateWithIO;
...
public class InternalTsCursor {
    ...
    private static <X, Y> Y applyNotNullWithIO(String funcName, FunctionWithIO<X, Y> func, X input) throws IOException, RuntimeException {
        Y result = func.apply(input);
        if (result != null) {
            return result;
        }
    ...
    public static final class MappingCursor<ID, Z> extends ForwardingCursor<Z> {
        ...
        private final FunctionWithIO<? super ID, ? extends Z> function;

        public MappingCursor(
                @Nonnull TsCursor<ID> delegate,
                @Nonnull FunctionWithIO<? super ID, ? extends Z> function) {
    ...
    public static final class FilteringCursor<ID> extends ForwardingCursor<ID> {
        ...
        private final PredicateWithIO<? super ID> filter;

        public FilteringCursor(
                @Nonnull TsCursor<ID> delegate,
                @Nonnull PredicateWithIO<? super ID> filter) {
            super(delegate);
    ...
    public static final class EmptyCursor<ID> extends InMemoryCursor<ID> {
        ...
        @Override
        public EmptyCursor<ID> filter(PredicateWithIO<? super ID> predicate) {
            requireNonNull(predicate, ID_FILTER_NPE);
            return this;
        }

        @Override
        public <Z> EmptyCursor<Z> map(FunctionWithIO<? super ID, ? extends Z> function) {
    ...
    public static final class SingletonCursor<ID> extends IteratingCursor<ID, ID> {
        ...
        public SingletonCursor(
            ...
            super(Collections.singleton(id).iterator(), FunctionWithIO.identity(), o -> data, o -> meta, o -> label);
    ...
    public static class IteratingCursor<E, ID> extends InMemoryCursor<ID> {
        ...
        private FunctionWithIO<? super E, ? extends ID> toId;
        ...
        public IteratingCursor(
                ...
                @Nonnull FunctionWithIO<? super E, ? extends ID> toId,
        ...
        @Override
        public IteratingCursor<E, ID> filter(PredicateWithIO<? super ID> predicate) {
            iterator = compose(iterator, toId, requireNonNull(predicate, ID_FILTER_NPE));
            return this;
        }

        @Override
        public <Z> IteratingCursor<E, Z> map(FunctionWithIO<? super ID, ? extends Z> function) {
    ...
    private static <E, ID> Iterator<E> compose(Iterator<E> iterator, FunctionWithIO<? super E, ? extends ID> toId, PredicateWithIO<? super ID> predicate) {
        return filter(iterator, PredicateWithIO.unchecked(o -> predicate.test(applyNotNullWithIO("id", toId, o))));
    }
