 ...
 * @author Jean Palate
 */
public class StlSpecification {

    public static StlSpecification defaultSpec(int np, int swindow, boolean robust) {
        StlSpecification spec = new StlSpecification(np);
        if (robust) {
            spec.no = 15;
            spec.ni = 1;
        } else {
    ...
    public StlSpecification(int np) {
        if (np == 1) {
