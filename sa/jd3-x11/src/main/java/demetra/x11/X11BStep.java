...
public class X11BStep {
    ...
    private void b4(X11Context context) {
        IFiltering filter = X11SeasonalFiltersFactory.filter(context.getPeriod(), SeasonalFilterOption.S3X3);
    ...
    private void b9(X11Context context) {
        IFiltering filter = X11SeasonalFiltersFactory.filter(context.getPeriod(), SeasonalFilterOption.S3X5);
...
//        TsData b13 = toolkit.getContext().op(b11, b7);
    ...
    private double[] table(int n, double value) {
