    ...
    public void estimate(IEstimationProblem problem) {
		...
		// scale the variables according to the norms of the columns
            ...
            for (double ratio = 0; ratio < 1.0e-4;) {
                ...
                if (!updateResidualsAndCost(problem))
                    break;
		...
		// tests for termination and stringent tolerances
                ...
                if ((Math.abs(actRed) <= 2.2204e-16) && (preRed <= 2.2204e-16)
                    ...
                    throw new FunctionException(
                            "Minpack: cost relative tolerance is too small");
                } else if (delta <= 2.2204e-16 * xNorm) {
                    throw new FunctionException(
                            "Minpack: parameters relative tolerance is too small");
                } else if (maxCosine <= 2.2204e-16) {
                    throw new FunctionException(
                            "Minpack: orthogonality tolerance is too small");
                }
            }
        }
