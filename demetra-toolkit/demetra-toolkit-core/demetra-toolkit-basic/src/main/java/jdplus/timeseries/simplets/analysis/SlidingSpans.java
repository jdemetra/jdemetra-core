...
public class SlidingSpans<I extends Explorable> {
	...
	for (int i = 0; i < m_estimation.length; ++i) {
	    ...
	    if (data != null)
		switch (info) {
		case PeriodToPeriodGrowthDifference -> addPct(1, buffer, data);
		case AnnualGrowthDifference -> addPct(data.getAnnualFrequency(), buffer, data);
		case PeriodToPeriodDifference -> addDel(1, buffer, data);
		case AnnualDifference -> addDel(data.getAnnualFrequency(), buffer, data);
		default -> addValue(buffer, data);
		}
	}
