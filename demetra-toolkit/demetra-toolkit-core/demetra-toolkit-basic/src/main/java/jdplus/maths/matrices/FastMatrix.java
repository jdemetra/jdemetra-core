...
public interface FastMatrix extends Matrix.Mutable{
     ...
     * @return
     */
    public static FastMatrix ofInternal(FastMatrix matrix) {
        if (matrix == null) {
            return null;
        }
    ...
    public static String format(FastMatrix matrix, String fmt) {
        StringBuilder builder = new StringBuilder();
        if (!matrix.isEmpty()) {
            DataBlock row = matrix.row(0);
            builder.append(row.toString(fmt));
    ...
    public static String format(FastMatrix m) {
        StringBuilder builder = new StringBuilder();
        int nrows = m.getRowsCount();
        if (nrows > 0) {
            builder.append(m.row(0).toString());
            for (int r = 1; r < nrows; ++r) {
                builder.append(System.lineSeparator());
                builder.append(m.row(r).toString());
            }
        }
        return builder.toString();
    }
