 ...
 */
package internal.jdplus.math.functions.riso;
 ...
 *
 * LBFGS is distributed as part of the RISO project. Following is a message from
 * Jorge Nocedal:
 *
 * <pre>
 ...
 * the Fortran code have been pasted in here as well. </p>
 *
 * <p>
 ...
 * the Netlib blurb on the Fortran source. </p>
 *
 * <pre>
...
public class Lbfgs {
    ...
    static interface IFeedBack {
         ...
         */
        void show(int iter, double[] x, double f, double[] g);
    }
     ...
     * @return
     */
    public static double ddot(int n, double[] dx, int ix0, int incx,
            double[] dy, int iy0, int incy) {
        double dtemp;
        int i, ix, iy, m, mp1;
    ...
    private double gnorm = 0, stp1 = 0, ftol = 0, stp[] = new double[1],
            ys = 0, yy = 0, sq = 0, yr = 0, beta = 0, xnorm = 0;
    private int iter = 0, nfun = 0, point = 0, ispt = 0, iypt = 0, maxfev = 0,
     ...
     *
     * Progress messages are printed to <code>System.out</code>, and non-fatal
     * error messages are printed to <code>System.err</code>. Fatal errors cause
     * exception to be thrown, as listed below.
     *
     * @param n The number of variables in the minimization problem.
     * Restriction: <code>n &gt; 0</code>.
     *
     * @param m The number of corrections used in the BFGS update. Values of
     * <code>m</code> less than 3 are not recommended; large values of
     * <code>m</code> will result in excessive computing time.
     ...
     * <code>iflag = 0</code>, it contains the values of the variables at the
     * best point found (usually a solution).
     *
     * @param f Before initial entry and on a re-entry with
     * <code>iflag = 1</code>, it must be set by the user to contain the value
     * of the function <code>f</code> at the point <code>x</code>.
     *
     * @param g Before initial entry and on a re-entry with
     * <code>iflag = 1</code>, it must be set by the user to contain the
     * components of the gradient <code>g</code> at the point <code>x</code>.
     *
     * @param diagco Set this to <code>true</code> if the user wishes to provide
     * the diagonal matrix <code>Hk0</code> at each iteration. Otherwise it
     * should be set to <code>false</code> in which case <code>lbfgs</code> will
     * use a default value described below. If <code>diagco</code> is set to
     * <code>true</code> the routine will return at each iteration of the
     * algorithm with <code>iflag = 2</code>, and the diagonal matrix
     * <code>Hk0</code> must be provided in the array <code>diag</code>.
     *
     * @param diag If <code>diagco = true</code>, then on initial entry or on
     * re-entry with <code>iflag = 2</code>, <code>diag</code> must be set by
     * the user to contain the values of the diagonal matrix <code>Hk0</code>.
     * Restriction: all elements of <code>diag</code> must be positive.
     *
     * @param iprint Specifies output generated by <code>lbfgs</code>.
     * <code>iprint[0]</code> specifies the frequency of the output: <ul> <li>
     * <code>iprint[0] &lt; 0</code>: no output is generated, <li>
     * <code>iprint[0] = 0</code>: output only at first and last iteration, <li>
     * <code>iprint[0] &gt; 0</code>: output every <code>iprint[0]</code>
     * iterations. </ul>
     *
     * <code>iprint[1]</code> specifies the type of output generated: <ul> <li>
     * <code>iprint[1] = 0</code>: iteration count, number of function
     * evaluations, function value, norm of the gradient, and steplength, <li>
     * <code>iprint[1] = 1</code>: same as <code>iprint[1]=0</code>, plus vector
     * of variables and gradient vector at the initial point, <li>
     * <code>iprint[1] = 2</code>: same as <code>iprint[1]=1</code>, plus vector
     * of variables, <li> <code>iprint[1] = 3</code>: same as
     * <code>iprint[1]=2</code>, plus gradient vector. </ul>
     *
     ...
     *
     * @param xtol An estimate of the machine precision (e.g. 10e-16 on a SUN
     * station 3/60). The line search routine will terminate if the relative
     * width of the interval of uncertainty is less than <code>xtol</code>.
     *
     * @param iflag This must be set to 0 on initial entry to
     * <code>lbfgs</code>. A return with <code>iflag &lt; 0</code> indicates an
     * error, and <code>iflag = 0</code> indicates that the routine has
     ...
     * are possible: <ul> <li> <code>iflag = -1</code> The line search routine
     * <code>mcsrch</code> failed. One of the following messages is printed:
     * <ul> <li>Improper input parameters. <li>Relative width of the interval of
     * uncertainty is at most <code>xtol</code>. <li>More than 20 function
     * evaluations were required at the present iteration. <li>The step is too
     * small. <li>The step is too large. <li>Rounding errors prevent further
     * progress. There may not be a step which satisfies the sufficient decrease
     * and curvature conditions. Tolerances may be too small. </ul>
     * <li><code>iflag = -2</code> The i-th diagonal element of the diagonal
     * inverse Hessian approximation, given in DIAG, is not positive.
     * <li><code>iflag = -3</code> Improper input parameters for LBFGS
     * (<code>n</code> or <code>m</code> are not positive). </ul>
     * @throws ec.tstoolkit.maths.realfunctions.riso.Lbfgs.ExceptionWithIflag
     *
     */
    public void lbfgs(int n, int m, double[] x, double f, double[] g,
        ...
        while (true) {
            ...
            mcsrch.fn(this, n, x, f, g, w, ispt + point * n, stp, ftol, xtol,
