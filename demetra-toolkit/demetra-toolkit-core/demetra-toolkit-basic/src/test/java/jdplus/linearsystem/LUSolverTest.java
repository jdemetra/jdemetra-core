...
public class LUSolverTest {
    ...
    public void testRandom() {
        ...
        LUSolver solver = LUSolver.builder().decomposer((A, e) -> CroutDoolittle.decompose(A, e)).build();
        solver.solve(M, x);
    ...
    public void testSingluar() {
        ...
        x.set(rnd::nextDouble);
        DataBlock y = x.deepClone();
        LUSolver2 solver2 = new LUSolver2(1e-13);
        boolean ok=true;
        try{
        solver2.solve(M, x);
         ...
         assertTrue(!ok);
        LUSolver solver = LUSolver.builder().decomposer((A, e) -> CroutDoolittle.decompose(A, e)).precision(1e-13).build();
        ok=true;
        try{
        solver.solve(M, x);
        }catch (MatrixException err){
    ...
    public static void main(String[] args) {
        ...
        for (int k = 0; k < K; ++k) {
            LUSolver solver = LUSolver.builder().decomposer((A, e) -> Gauss.decompose(A, e)).build();
            solver.solve(M, x.deepClone());
        }
        ...
        t0 = System.currentTimeMillis();
        for (int k = 0; k < K; ++k) {
            new LUSolver2().solve(M, x.deepClone());
        }
        t1 = System.currentTimeMillis();
        System.out.println(t1 - t0);
    }
