...
public class HtmlRegArima extends AbstractHtmlElement {
    ...
    public void writeArima(HtmlStream stream) throws IOException {
        ...
        DoubleSeqCursor vars = model.getEstimation().getCoefficientsCovariance().diagonal().cursor();
        double vcorr = (nobs - nparams) / (nobs - nparams + nhp);
    ...
    private <V extends ITsVariable> void writeFixedRegressionItems(HtmlStream stream, String header, TsDomain context, Predicate<Variable> predicate) throws IOException {
        ...
        for (Variable reg : regs) {
            for (int i = 0; i < reg.dim(); ++i) {
                ...
                if (c[i].isFixed()) {
                    stream.open(HtmlTag.TABLEROW);
                    stream.write(new HtmlTableCell(reg.getCore().description(i, context)).withWidth(100));
